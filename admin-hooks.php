 ...
 * Make sure to only handle this when old status is unapproved and new status is approved.
 ...
 * @since 2.1.1
 */
function zeno_report_comments_comment_unapproved_to_approved( $comment ) {

	$comment_id = $comment->comment_ID;

	$reports = (int) get_comment_meta( $comment_id, 'zrcmnt_reported', true );
	if ( $reports > 0 ) { // to avoid unapproved comments when moderation is enabled on new comments.
		update_comment_meta( (int) $comment_id, 'zrcmnt_moderated', true );
		delete_comment_meta( (int) $comment_id, 'zrcmnt_reported' );
	}

}
add_action( 'comment_unapproved_to_approved', 'zeno_report_comments_comment_unapproved_to_approved', 10, 1 );
